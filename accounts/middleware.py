...
class MoveJWTCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if request.path in [reverse("accounts:token_verify"), reverse("accounts:rest_logout")] and settings.JWT_AUTH_COOKIE in request.COOKIES:
            if request.body != b'':
...
class MoveJWTRefreshCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if request.path in [reverse("accounts:token_refresh"), reverse("accounts:rest_logout")] and settings.JWT_AUTH_REFRESH_COOKIE in request.COOKIES:
            if request.body != b'':
                data = json.loads(request.body)
